...
/**
 * Màn hình danh sách các tập Podcast Yêu thích (Favorites Screen):
 * - Edge-to-Edge tràn viền với nút Back tròn nổi bán trong suốt.
 * - Danh sách các tập podcast mà người dùng đã thả tim ❤️.
 */
@Composable
fun PodcastFavoritesScreen(
    onBackClick: () -> Unit = {},
    ...
    Scaffold(
        ...
        Box(
            ...
            LazyColumn(
                ...
                // Immersive Hero Header với ảnh favorite.webp + Pill Bar 3 Nút UI + Dynamic Background Color
                item(key = "header") {
                    ...
                    val isFavPlaying = playerIsPlaying && favoriteEpisodes.any { it.episode.id == currentPlayingId }

                    LibraryDetailHeader(
                        title = "Tập yêu thích",
                        subtitle = "${favoriteEpisodes.size} tập podcast đã thả tim",
                        imagePath = imagePath,
                ...
                if (favoriteEpisodes.isEmpty()) {
                    item(key = "empty") {
                        EmptyState(
                            message = "Chưa có tập podcast yêu thích",
                        ...
                        val dismissState = rememberSwipeToDismissBoxState(
                            confirmValueChange = { value ->
                                if (value == SwipeToDismissBoxValue.EndToStart) {
                                    viewModel.toggleFavorite(item.episode.id)
                                    true
                                } else false
                            }
                        )
